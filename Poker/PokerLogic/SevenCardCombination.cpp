...
FiveCardCombination SevenCardCombination::find_best(const std::vector<Card>& vec, const std::vector<Card>& add_vec) {
	if ((vec.size() != 5) || (add_vec.size() != 2)) { throw std::runtime_error("Not five cards for combination"); }
...
FiveCardCombination SevenCardCombination::cast_string(const std::string& str, const std::string& add_str) {
	if ((str.size() != 14) || (add_str.size() != 5)) { throw std::runtime_error("Not five cards for combination"); }
...
SevenCardCombination::SevenCardCombination(const std::vector<Card>& vec) 
	: SevenCardCombination::SevenCardCombination({ vec[0], vec[1], vec[2], vec[3], vec[4] }, { vec[5], vec[6] }) 
{
	if (vec.size() != 7) { throw std::runtime_error("Not five cards for combination"); }
}
